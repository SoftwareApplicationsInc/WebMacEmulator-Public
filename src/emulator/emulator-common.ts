...
export type EmulatorWheelEvent = { type: "wheel"; deltaX: number; deltaY: number };
...
export function generateNextChunkIndex(
    url: string,
    specs: EmulatorChunkedFileSpec[]
): [EmulatorChunkedFileSpec, number] | undefined {
    ...
    for (const inputEvent of inputEvents) {
        switch (inputEvent.type) {
            ...
            case "mouseup":
                ...
                break;
            case "wheel":
                // Filter out wheel events with small delta values (these may be automatically generated e.g. to represent deceleration, or reflect very small user movements)
                // This is a poor implementation and should be replaced with some sort of debounce mechanism
                if (Math.abs(inputEvent.deltaY) < 4)
                    break;

                remainingEvents.push({ type: "keydown", keyCode: (inputEvent.deltaY > 0 ? 61 : 62) });
                remainingEvents.push({ type: "keyup", keyCode: (inputEvent.deltaY > 0 ? 61 : 62) });
                break;
            case "keydown":
            case "keyup":
                if (hasKeyEvent) {
