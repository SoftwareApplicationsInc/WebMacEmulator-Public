...
import PrefetchWorker from "./emulator-worker-prefetch?worker";
...
export interface EmulatorEthernetProviderDelegate {
    ...
    requestOpenURL?(url: string): void;
    mailingListSignupStarted(textFieldRect: MacRect): void;
    mailingListSignupCancelled(): void;
    mailingListSignupInvalidAddressEntered(): void;
    mailingListSignupFinished(emailAddress: string): void;
}
...
export interface EmulatorDelegate {
    ...
    emulatorDidStartMailingListSignup?(emulator: Emulator, textFieldRect: MacRect): void;
    emulatorDidCancelMailingListSignup?(emulator: Emulator): void;
    emulatorDidWarnForInvalidMailingListAddress?(emulator: Emulator): void;
    emulatorDidFinishMailingListSignup?(
        emulator: Emulator,
        emailAddress: string
    ): void;
...
export class Emulator {
    ...
    constructor(config: EmulatorConfig, delegate?: EmulatorDelegate) {
        ...
        config.ethernetProvider?.setDelegate({
            ...
            requestOpenURL: (url: string) => {
                this.#delegate?.emulatorDidRequestOpeningURL?.(this, url);
            },
            mailingListSignupStarted: (textFieldRect: MacRect) => {
                this.#delegate?.emulatorDidStartMailingListSignup?.(this, textFieldRect);
            },
            mailingListSignupCancelled: () => {
                this.#delegate?.emulatorDidCancelMailingListSignup?.(this);
            },
            mailingListSignupInvalidAddressEntered: () => {
                this.#delegate?.emulatorDidWarnForInvalidMailingListAddress?.(this);
            },
            mailingListSignupFinished: (emailAddress: string) => {
                this.#delegate?.emulatorDidFinishMailingListSignup?.(this, emailAddress);
            }
        });
    ...
    async #startWorker() {
        ...
        this.#prefetchWorker.postMessage({ type: "port", port: channel.port2 }, [channel.port2]);
        ...
        const config: EmulatorWorkerConfig = {
            ...
            autoloadFiles: {
                ...
                ".basilisk_ii_xpram": hexToBytes(""),
            },
            ...
            prefetch: channel.port1,
        };
        for (const spec of config.disks.concat(config.delayedDisks ?? [])) {
            config.prefetch.postMessage({ type: "prefetch", spec });
        }
    ...
    #handleKeyDown = (event: KeyboardEvent) => {
        if (event.target instanceof HTMLInputElement && !event.target.classList.contains("Mac-Keyboard-Input")) {
            return;
        }
        event.preventDefault();
        const { code, keyCode } = event;
        let synthesizeShiftKeyEvent = false;
        const charactersRequiringShiftKey = "!@#$%^&*()_+{}|:\"<>?~"
        if (!event.shiftKey && charactersRequiringShiftKey.includes(event.key)) {
            // iOS does not send shift key events for special characters, so we need to insert them ourselves if needed.
            this.#input.handleInput({
                type: "keydown",
                keyCode: JS_CODE_TO_ADB_KEYCODE["ShiftLeft"],
            });
            synthesizeShiftKeyEvent = true;
        }
