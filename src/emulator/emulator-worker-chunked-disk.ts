...
export class EmulatorWorkerChunkedDisk implements EmulatorWorkerDisk {
    ...
    #prefetch: MessagePort | undefined;
    ...
    protected doChunkRequest(
        ...
        if (this.#prefetch) {
            const response = new SharedArrayBuffer(spec.chunkSize);
            const sharedLock = new SharedArrayBuffer(4);
            const lock = new Int32Array(sharedLock);
            this.#prefetch.postMessage({ type: "load", chunkUrl, lock: sharedLock, response });
            Atomics.wait(lock, 0, 0);
            if (Atomics.load(lock, 0) === 1) {
                return {chunk: new Uint8Array(response)};
            }
        }
        ...
        if (this.#prefetch) {
            this.#prefetch.postMessage({ type: "store", chunkUrl, chunk })
        } 
