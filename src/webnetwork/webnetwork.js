...
// AGPL3, see https://github.com/oldweb-today/oldweb-today/blob/main/LICENSE
...
(function () {
  ...
  const messagePortStream = (port) => {
    let writableClosed, readableClosed;
    const closed = Promise.all([
      new Promise(r => writableClosed = r),
      new Promise(r => readableClosed = r).then(() => port.onmessage = null),
    ]).then(() => port.close());
    ...
    port.onmessage = ({ data }) => readController.enqueue(data);
  ...
  const makeTransformStreamDefaultController = (writeController, readController) => {
    const _this = Object.create(TransformStreamDefaultController$1.prototype);
    ...
    return (
      function (Module) {
        ...
        if (Module['locateFile']) {
          ...
          wasmBinaryFile = new URL('picotcp.wasm', import.meta.url).href;
        }*/
        var wasmBinaryFile = new URL('picotcp.wasm', import.meta.url).href;
        console.log("Loading", wasmBinaryFile);
  ...
  */
   ...
   */
  class IPv4Parser extends TransformStream$1 {
    /**
    ...
    static transform(lower, controller) {
      ...
      // TODO: options.
      Object.assign(ret, {
        payload: toHexString(payload.subarray(headerLength, ret.length)),
        lower,
      });
   ...
   */
  class EthernetParser extends TransformStream$1 {
  ...
  */
  ...
  let PORT = 8080;
  let initialArticleName = "";
  let homeUrl;
  let controlSocketWriter = null;
  let broadcastFromEmulatorChannelPort = null;
  let broadcastToEmulatorChannelPort = null;
  let controlChannelPort = null;

  self.onmessage = (event) => {
    if (event.data.broadcastFromEmulatorChannelPort) {
      broadcastFromEmulatorChannelPort = event.data.broadcastFromEmulatorChannelPort;
    }
    if (event.data.broadcastToEmulatorChannelPort) {
      broadcastToEmulatorChannelPort = event.data.broadcastToEmulatorChannelPort;
    }
    if (event.data.controlChannelPort) {
      controlChannelPort = event.data.controlChannelPort;
      controlChannelPort.onmessage = (event) => {
        sendLoadArticleCommand(event.data.loadArticleName);
      };
    }
    if (event.data.initialArticleName) {
      initialArticleName = event.data.initialArticleName;
    }
  ...
  function sendLoadArticleCommand(articleName) {
    if (controlSocketWriter) {
      console.log("[SAIControlConnection] Sending L request with payload \"" + articleName + "\"");
      const encoder = new TextEncoder();
      controlSocketWriter.write(encoder.encode("L"+articleName));
    } else {
      console.log("[SAIControlConnection] Not sending L request because client app is not running");
    }
  }

  async function main() {
    const nic = await new NIC(undefined, new Uint8Array([178, 250, 80, 37, 2, 130]));

    let readableNic = nic.readable;
    let readableMessagePortStream = messagePortStream(broadcastFromEmulatorChannelPort).readable;
    let readableNicStreams = readableNic.tee();
    let readableMessagePortStreams = readableMessagePortStream.tee();
  
    readableNicStreams[0].pipeThrough(messagePortStream(broadcastToEmulatorChannelPort));
    readableMessagePortStreams[0].pipeThrough(nic);

    monitorStream(readableNicStreams[1], " -> ");
    monitorStream(readableMessagePortStreams[1], " <- ");

    nic.addIPv4(SERVER);

    await nic.startDHCPServer(SERVER, "255.255.255.0");

    const proxyServer = new nic.TCPServerSocket({ localPort: PORT, localAddress: SERVER });
    const controlServer = new nic.TCPServerSocket({ localPort: 5357, localAddress: SERVER });
...
`;
    ...
    async function handleProxyServerResponse(socket) {
      ...
      const fetchUrl = "https://classic-mac-website-cors-proxy-648cb638402a.herokuapp.com/" + targetUrl;
      ...
      sendHTTPResponse({ content, status, statusText, contentType, writer });
    }

    const encoder = new TextEncoder();

    function sendHTTPResponse({ writer, content, status = 200, statusText = "OK", contentType = "text/plain" }) {
...
\r\n`));
    ...
    const controlSocketHandler = async (controlSocket, handleResponse) => {
      console.log("[SAIControlConnection] Connection opened");
      const reader = controlSocket.readable.getReader();
      const writer = controlSocket.writable.getWriter();
      controlSocketWriter = writer;
      sendLoadArticleCommand(initialArticleName);
      while (true) {
          const { done, value } = await reader.read();
          if (done) {
              break;
          } else {
              handleSocketData(value);
          }
      }
      console.log("[SAIControlConnection] Connection closed");
      controlChannelPort.postMessage({ type: "browserConnectionClosed" });
      reader.releaseLock();
      writer.releaseLock();
      controlSocketWriter = null;
    };

    async function handleControlServerResponse(socket) {
      controlSocketHandler(socket);
    }

    async function handleSocketData(data) {
      const req = new TextDecoder().decode(data);
      const clientControlMessageType = req[0];
      let clientControlPayload = req.substring(1);
      // Strip trailing null byte from payload if present
      if (clientControlPayload[clientControlPayload.length - 1] === "\0") {
        clientControlPayload = clientControlPayload.substring(0, clientControlPayload.length - 1);
      }
      console.log("[SAIControlConnection] Got client request of type " + clientControlMessageType + " with payload " + clientControlPayload);
      if (clientControlMessageType === "N") {
        controlChannelPort.postMessage({ type: "browserNavigated", articleName: clientControlPayload });
      } else if (clientControlMessageType === "O") {
        controlChannelPort.postMessage({ type: "newWindow", url: clientControlPayload });
      } else if (clientControlMessageType === "S") {
        controlChannelPort.postMessage({ type: "mailingListSignupStarted", payload: clientControlPayload });
      } else if (clientControlMessageType === "C") {
        controlChannelPort.postMessage({ type: "mailingListSignupCancelled" });
      } else if (clientControlMessageType === "I") {
        controlChannelPort.postMessage({ type: "mailingListSignupInvalidAddressEntered" });
      } else if (clientControlMessageType === "M") {
        controlChannelPort.postMessage({ type: "mailingListSignupFinished", emailAddress: clientControlPayload });
      }
    }

    const readServer = async (server, handleResponse) => {
    ...
    readServer(proxyServer, handleProxyServerResponse);
    readServer(controlServer, handleControlServerResponse);
  }

  const printer = (tag, ...args) => new TransformStream$1({
    transform(v, c) {
  ...
  async function monitorStream(readable, label) {
    readable
      .pipeThrough(new EthernetParser)
      .pipeThrough(printer("ethernet " + label))
      .pipeThrough(new IPv4Parser)
